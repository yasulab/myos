...
void HariMain(void)
{
	...
	*((int *) 0x0fec) = (int) &fifo;
	...
	memtotal = memtest(start_addr, end_addr);	/* 使用できるメモリ領域を確認 */
	memman_init(memman);

	/* 起動に使用したメモリだが, もう使わないので解放する */
	memman_free(memman, 0x00001000, 0x0009e000); /* 0x00001000 - 0x0009efff */
	memman_free(memman, start_addr, memtotal - start_addr);	/* メモリマネージャに渡すメモリ領域 */
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	keywin_on(key_win);	// カーソルをconsoleに合わせる
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) { /* キーボードデータ */
				...
				if (i == 256 + 0x3d && key_shift != 0) {	/* Shift+F3 */
					/* ログ用windowを作成する */
					/*
					key_win = open_log(shtctl, memtotal);
					sheet_slide(key_win, 512, 4);
