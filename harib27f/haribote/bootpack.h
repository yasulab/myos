...
#define MEMMAN_ADDR			0x003c0000	/* Memory Managerのメモリ番地*/
...
#define LAST_DATA_ADDR	0x00600000	// データ管理領域の終わり+1
#define MAX_FINFO_NUM 50	// ファイル情報の最大登録数
#define MAX_NAME_LENGTH 8	// ディレクトリ名/ファイル名の最大文字数

/* finfo->fdata->head.char用のビット配列
 * 0x08, 0x10, 0x20, 0x40, 0x80は未使用  */
#define STAT_ALL	0xFF	/* 排他制御用 */
#define STAT_VALID	0x01	/* valid bit */
#define STAT_CONT	0x02	/* continuous bit */
#define STAT_BUF	0x04	/* buffer bit:バッファ領域にあるファイルデータであることを示す */
#define STAT_OPENED	0x08	/* file openされているかどうかを示す(デフォルトは0) */

/*　finfo->type用のビット配列(現状では, ディレクトリとファイルの２種類しか区別していない)
 * 0x10: ディレクトリ情報
 * 0x20: ファイル情報
 * 0x40: システムファイル
 * 0x80: それ以外の情報(ディスクの名前とか)*/
...
/* ブロックに関する定義 */
#define BLOCK_SIZE 1024	// ファイルデータのサイズ(defaultは4,096)
#define BODY_SIZE (BLOCK_SIZE - sizeof(struct HEAD))	//1ブロックあたりの実データのサイズ
#define BODY_SIZE_OFFSET 128	// 余分に確保するサイズ(無くてもよい？)
#define MAX_BLOCK_NUM 50 // 最大ブロック数(これ以上のブロックの確保は許さない)
struct FILEINFO {
...
struct MYDIRINFO {
	...
	struct MYDIRINFO *this_dir; // これは別になくても問題無いはず
};
...
struct HEAD{
	unsigned char stat;
	char name[12];
	struct MYFILEDATA *this_fdata;	// データ領域のアドレス
	struct MYDIRINFO *this_dir;		// 含有しているディレクトリのアドレス
	struct MYFILEDATA *next_fdata;	// 次のファイルデータ(データ上限を超えていた場合使用する)
	// next_dataのデフォルトの値は0x0000 0000(エンドflag bitが立っている時と同じ)
	/* stat is a bit arguments shown below
	 * valid bit: 0x01
	 * continuous bit: 0x02　(続きがあるファイル。複数のファイルデータから構成されている)
	 * end file: 0x04 (いらない？)
	 * opened file 0x08
	 */
};
struct MYFILEDATA{
	struct HEAD head;
	char body[BODY_SIZE];	// 合計サイズが1024byteになるようにする
};
...
/* console.c */
#define MAX_CMDLINE	50	// コマンドラインの入力文字列上限
